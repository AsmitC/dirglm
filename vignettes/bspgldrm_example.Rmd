...
```{r}
# muHat on boundary of spt
set.seed(100)
# Note: without intercept, expect mu = y i.e., beta = c(0, 1)
# but getTheta fails via floating point issue I think.
# Not sure how gldrm is getting past that
n <- 10
...
gldrm:::getTheta(
     spt = c(0, 1),
     f0 = c(0.5, 0.5),
     mu = c(rep(-2.77555756156289e-17, 5), rep(1, 5)),
     sampprobs = NULL,
     ySptIndex = NULL,
     thetaStart = NULL
)
...
colMeans(m1$samples$beta) # Expect c(0.5, NA)
colmeans(m2$samples$beta) # Expect c(0, NA)
```
