...
plot_cdpglm <- function(
  ...
  if (what == "rho") {
    rho_samples <- as.numeric(fit$samples$rho)

    alpha <- (1 - prob) / 2

    if (robust) {
      center <- stats::median(rho_samples, na.rm = TRUE)
    } else {
      center <- mean(rho_samples, na.rm = TRUE)
    }

    lower <- stats::quantile(rho_samples, alpha, na.rm = TRUE)
    upper <- stats::quantile(rho_samples, 1 - alpha, na.rm = TRUE)

    dj <- do.call(stats::density, c(list(x = rho_samples), dens_args))

    if (is.null(xlab)) xlab <- expression(rho)
    if (is.null(ylab)) ylab <- "Density"
    if (is.null(main)) {
      main <- sprintf(
        "Posterior %s of rho (%.0f%% CrI shaded)",
